...
#-----------------------------------------------   Adding By Sudipta ------------------------#
class EarlyConvDS_56(nn.Module):
    def __init__(
        self,
        dim,
    ):
...
class EarlyConvDS_112(nn.Module):
    """
    per block design of "FasterViT : Hatamizadeh et al"
    """
    def __init__(
...
#------------------------------------------------------------------------------------------------------#
#------------------------------------------------------------------------------------------------------#
class EarlyConvDS_28(nn.Module): # By Vignesh
    def __init__(
        self,
        dim,
    ):
...
class EarlyConvDS_14(nn.Module): # By Vignesh
    def __init__(
        self,
        dim,
    ):
...
class MultiConvEmbed(nn.Module):
    def __init__(
        ...
        self.conv0 = nn.Sequential(                                                             # Two Time Conv O/p with k = 3, s= 2 and p = 1 -->> size/4 apporx. and 2x the chanel dim (embed_dim=192)
            nn.Conv2d(in_chans, in_dim, 3, 2, 1, bias=False),
            nn.BatchNorm2d(in_dim, eps=1e-4),                                                   # 64
            nn.ReLU(),
        ...
        self.conv1 = nn.Sequential(                                                         # Residual No Changed in Shape
            EarlyConvResidual(embed_dim),
            EarlyConvResidual(embed_dim),
            EarlyConvResidual(embed_dim),
        )
        self.ds1 = EarlyConvDS(embed_dim)                                                   # changed in shape with single time Conv O/p with k = , s= 2 and p = 1 --->> Size/2 (approx) and 2x the chanel dim (now embed_dim=192)
        self.conv2 = nn.Sequential(                                                         # Residual No Changed 
            EarlyConvResidual(2 * embed_dim),
            EarlyConvResidual(2 * embed_dim),
            EarlyConvResidual(2 * embed_dim),
        )
        self.ds2 = EarlyConvDS(2 * embed_dim)                # changed in channel dim 2x and shape with single time Conv O/p with k = 3, s= 2 and p = 1 --->> Size/2 (approx)
#------------------ Added By Sudipta -----------------------------------------------        
        self.ds3 = EarlyConvDS_56(2 * embed_dim) 
        self.ds4 = EarlyConvDS_112(embed_dim)               # Simliar to ds1                                       
#------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------
        self.ds5 = EarlyConvDS_28(2 * embed_dim)            # By Vignesh -handles (512x512 and 448x448)
        self.ds6 = EarlyConvDS_14(2 * embed_dim)            # By Vignesh -handles (256x256 and 224x224)
#------------------------------------------------------------------------------------
        self.flatten = flatten

    def forward(self, x):
        #--------------------- Padding (Not Needed)------------------#
        #x = F.pad(x, pad=(64, 64, 64, 64), mode='constant', value=0)                                        # Padding [30, 3, 896, 896]  -->> [30, 3, 1024, 1024] 
        #print("x.shape at first = ",x.shape)  # [30, 3, 1024, 1024]            [30, 3, 896, 896]
        #exit(0)        
        x = x.squeeze(2)
        #print("x.shape at 140 = ",x.shape)  # [30, 3, 1024, 1024]             [30, 3, 896, 896]  
        x = self.proj(x)
        #print("x.shape at 142 = ",x.shape)  # [30, 3, 1024, 1024]             [30, 3, 896, 896]
        x = self.conv0(x)
        #print("x.shape at 144 = ",x.shape)  # [30, 192, 256, 256]             [30, 192, 224, 224]
        x = self.conv1(x)
        #print("x.shape at 146 = ",x.shape)  # [30, 192, 256, 256]             [30, 192, 224, 224]
        x = self.ds1(x)         #x = self.ds4(x)  
        #print("x.shape at 148 = ",x.shape)  # [30, 384, 128, 128]             [30, 384, 112, 112]
        x = self.conv2(x)
        #print("x.shape at 150 = ",x.shape)  # [30, 384, 128, 128]             [30, 384, 112, 112]

 #------------------------ Add and Modify x =self.ds2(x) --- By Sudipta -------------------------------------

        if x.shape[-2:] == (112, 112):      # ([30, 768, 64, 64])
            x = self.ds3(x)
        elif x.shape[-2:] == (56, 56):      # ([30, 768, 32, 32]) # By Vignesh -handles (512x512 and 448x448)
            x = self.ds5(x)
        elif x.shape[-2:] == (28, 28):      # ([30, 768, 16, 16]) # By Vignesh -handles (256x256 and 224x224)
            x = self.ds6(x)
        else:
            x = self.ds2(x)
        #print("x.shape = ",x.shape)        # ([30, 768, 64, 64])              ([30, 768, 56, 56])

        #if x.shape[-2:] == (56, 56):
            #x = F.interpolate(x, size=(64,64), mode='bilinear', align_corners=False)                
        #print("x.shape = ",x.shape)        # ([30, 768, 64, 64])              ([30, 768, 56-->>64, 56--->>64])
        #exit(0)
#----------------------------------------------------------------------------------------------------
        if self.flatten:
...
class MultiScaleAttentionBlock(nn.Module):
    ...
    def propagate_bottom_up(
        ...
        for i in range(len(stages)-1):
            ...
            #print("Shape of current_stage:", current_stage.shape)      #([64, 768, 4, 4]) #[64, 768, 4, 4]
            #print("nw:", nw)                                           # 16  # 9          
            #nw = 16                           
            #b = current_stage.shape[0] 
            #c = current_stage.shape[1]
            #m1 = current_stage.shape[2]
            #m2 = current_stage.shape[3]

            #print(f"b: {b}, nw: {nw}, c: {c}, m1: {m1}, m2: {m2}")       #b: 64, nw: 16, c: 768, m1: 4, m2: 4   #  b:  64, nw: 9, c: 768, m1: 4, m2: 4
            #exit(0)
