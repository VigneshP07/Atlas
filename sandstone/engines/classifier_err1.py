...
class Classifier(Engine):
    ...
    def train_one_epoch(
        ...
        for batch_idx, batch in enumerate(
            ...
            if epoch_metrics_configured:
                ...
                # ✅ Guard against None before saving
                if predictions_dict.get("preds") is not None and predictions_dict.get("golds") is not None:
                    result.update({
                        "preds": predictions_dict["preds"].detach().cpu(),
                        "golds": predictions_dict["golds"].detach().cpu(),
                        "probs": predictions_dict.get("probs", None),
                    })
    ...
    def evaluate(self, model, dataloader, device, epoch=None, test=False, gather_predictions=False):
        ...
        return self.on_epoch_end(split="test" if test else "val", device=device, epoch=epoch)
    ...
    def step(self, model, batch, batch_idx, epoch=None, train=False, device="cuda"):
        ...
        with torch.amp.autocast("cuda", dtype=self.amp_precision, enabled=self.amp_precision is not None):
            ...
            loss, logging_dict = self.compute_step_metrics(
                loss_input={"logit": logit, "target": batch["y"], "batch": batch, "model_output": model_output},
                metric_input={"logit": logit, "target": batch["y"], "batch": batch, "model_output": model_output},
                train=train,
            )
