...
def build_experiment(args):
    ...
    #-----------------------------------Added----------------------------------------#
    num_total = sum(p.numel() for p in model_without_ddp.parameters())
    num_trainable = sum(p.numel() for p in model_without_ddp.parameters() if p.requires_grad)
    logger.info(f"Trainable params: {num_trainable:,} / {num_total:,} ({100.0 * num_trainable / num_total:.2f}%)")
    ...
    # optimizer and param groups
    trainable_named_params = [(n, p) for n, p in model_without_ddp.named_parameters() if p.requires_grad]
    assert len(trainable_named_params) > 0, "No trainable parameters found after freezing."

    if getattr(args.optimizer, "use_discriminative_lr", False):
        backbone_params = []
        head_params = []

        for name, p in trainable_named_params:
            if name.startswith("fc.") or name.startswith("fc_norm.") or name.startswith("readout_norm."):
                head_params.append(p)
            else:
                backbone_params.append(p)

        base_lr = args.optimizer.kwargs.lr
        backbone_lr = getattr(args.optimizer, "backbone_lr", base_lr * 0.1)
        head_lr = getattr(args.optimizer, "head_lr", base_lr)
    ...
    elif args.optimizer.get("layer_decay", None) is not None:
        if hasattr(model_without_ddp, "param_groups_lrd"):
            param_groups = model_without_ddp.param_groups_lrd(
                args.optimizer.timm_weight_decay,
            ...
            raise AttributeError("layer_decay requested but model has no param_groups_lrd()")

    elif args.optimizer.get("timm_weight_decay", None) is not None:
        all_groups = optim_factory.param_groups_weight_decay(
            model_without_ddp, args.optimizer.timm_weight_decay
        )
        filtered_param_groups = []
        ...
        param_groups = [p for _, p in trainable_named_params]
    #-----------------------------------Added----------------------------------------#
